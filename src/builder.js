const buildTrackPoints = (track, { calc }) => {
    const trackPoints = []
...
const buildLap = (lap, options) => {
    const { calc } = options

    // With calc feature
    if (calc) {
        const trackPoints = buildTrackPoints(lap.Track[0], options)
    ...
    return {
        ...
        totalTime: +lap.TotalTimeSeconds[0],
...
const buildActivity = (activity, options) => {
    return {
        id: activity.Id[0],
        sport: activity.$.Sport,
        laps: activity.Lap.map((l) => buildLap(l), options),
    }
}

const buildTraining = (training, options) => {
    return {
        options,
        activities: training.Activity.map((a) => buildActivity(a), options),
    }
}

const buildOneActivityOneLapTraining = (training, options) => {
    return {
        options,
        activity: {
            id: training.Activity[0].Id[0],
            sport: training.Activity[0].$.Sport,
            lap: buildLap(training.Activity[0].Lap[0], options),
        },
    }
}
